...
//    
    ...
    .selectAll('.left h4')
    .data(incomingData)
    .text(function (d) {console.log("Logging d"); console.log(d); return "..." + d.name.toString().slice(-10);})
//d3.select("#cpu").select('.left1 h4').html(incomingData[0].Names.toString().substring(0,16))
d3.select('#disk')
    .selectAll('.right h4')
    .data(incomingData)
    .text(function(d) {return humanFileSize(d.read, true).toString()+"/" + humanFileSize(d.write, true).toString();})
