...
export default class Table extends React.Component {
  ...
  _renderBodyRows() {
    ...
    this._currentPageDataItems().forEach((dataItem, dataItemIndex) => {
      ...
      if (matchedCustomRow) {
        ...
        var className = (this.props.selectedAppId === dataItem._id) ? 'active' : '';
        rows.push(<tr className={className} onClick={ () => { this._onClick({dataItem}) } } key={`row-${dataItemIndex}`}>{ allRowCells() }</tr>);
      }
      if (detailsExtraRow) {
  ...
  _onClick(row) {
    ...
    this.props.onSelectApplication(item._id);
  }
